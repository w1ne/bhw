// One place that sends mail for the production service.
//
// Club notifications go through the bhw-mailer Worker and Cloudflare Email
// Routing's send binding (see send() below), which carries no credential.
// The SMTP fallback sends from andrii@shylenko.com over PrivateEmail: that
// domain has a real mailbox and SPF, while bhw.hu has Email Routing MX/SPF
// but no mailbox or SMTP credential of its own. When the club has its own
// mailbox, this is the one file to change.
//
// Failure is logged and swallowed on purpose. The caller has already written
// its record to KV by the time this runs; a mail server having a bad minute
// must not undo a write we have already made.
//
//   npx wrangler pages secret put SMTP_PASSWORD --project-name bhw

import { sendMail } from "./smtp.js";

export const SMTP_HOST = "mail.privateemail.com";

// SENDER is where mail comes FROM: the PrivateEmail mailbox the SMTP account
// authenticates as, and the domain whose SPF record makes the mail survive a
// spam filter. OPS is where the club's copies go TO.
export const SENDER = "andrii@shylenko.com";
export const OPS = "shylenkoa@gmail.com";

// replyTo defaults to our own mailbox; an internal message about somebody
// passes their address instead, so hitting reply answers the person.
export async function send(env, to, subject, text, replyTo) {
  // The club's own copies go through the bhw-mailer Worker, which uses
  // Cloudflare Email Routing's send binding and carries no credential at all.
  // That binding can only deliver to the verified destination address on the
  // account, so this fork is for internal notifications and nothing else --
  // which is why the customer confirmation would still need SMTP.
  if (env.MAILER && to === OPS) {
    try {
      const response = await env.MAILER.fetch("https://mailer/", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ subject, text, replyTo: replyTo || "" }),
      });
      const data = await response.json().catch(() => null);
      if (data && data.ok) { console.log("[mail] sent via MAILER:", subject); return; }
      console.log("[mail] MAILER refused:", (data && data.error) || response.status, "-", subject);
    } catch (error) {
      console.log("[mail] MAILER failed:", error && error.message, "-", subject);
    }
    // Fall through: SMTP may still be configured and can carry it instead.
  }

  if (!env.SMTP_PASSWORD) { console.log("[mail] no SMTP_PASSWORD, skipping:", subject); return; }
  try {
    await sendMail({
      host: SMTP_HOST,
      user: env.SMTP_USER || SENDER,
      pass: env.SMTP_PASSWORD,
      from: env.SMTP_USER || SENDER,
      fromName: "Budapest Hardware Club",
      replyTo: replyTo || SENDER,
      to, subject, text,
    });
    console.log("[mail] sent:", subject, "->", to);
  } catch (error) {
    console.log("[mail] failed:", subject, "->", to, "-", error && error.message);
  }
}
